...
Run with: python -m pytest tests/test_database_quick.py
"""
...
def test_analyte_creation(db):
    ...
    with db.session_scope() as session:
        ...
        assert retrieved.molecular_weight == 58.08
...
def test_synonym_operations(db):
    ...
    # Query synonyms
    with db.session_scope() as session:
        synonyms = get_synonyms_for_analyte(session, analyte.id)
        ...
        exists = synonym_exists(session, analyte.id, "benzol")
        assert exists is True
        
        not_exists = synonym_exists(session, analyte.id, "nonexistent")
...
def test_lab_variant(db):
    ...
    with db.session_scope() as session:
        ...
        variant = create_lab_variant(
            ...
            reported_name="Acetone (2-Propanone)",
            method="EPA 8260D",
            matrix="Soil",
            units="µg/kg",
            frequency=15,
...
def test_match_decision(db):
    ...
    with db.session_scope() as session:
        ...
        decision = create_match_decision(
            ...
            top_k_candidates=[
                {"rank": 1, "analyte_id": analyte.id, "name": "Acetone", "score": 0.98}
            ],
            signals_used={
                "exact_match": 1.0,
                "levenshtein": 0.95,
...
def test_cascade_delete(db):
    ...
    with db.session_scope() as session:
        # Delete analyte
        from src.database.crud import delete_analyte
        deleted = delete_analyte(session, analyte_id)
        assert deleted is True
        
    with db.session_scope() as session:
        # Verify synonyms were also deleted
        synonyms = get_synonyms_for_analyte(session, analyte_id)
